...
//        $this->AddEvent('register', 'EventRegister');
    ...
    protected function EventAjaxLogin()
    {
         ...
         */
        $oUser = $this->User_GetUserByEmail(getRequest('login'));
        if (!$oUser) $oUser = $this->User_GetUserByPhone(NormalizePhone(getRequest('login')));
        if ($oUser) {
            /**
    ...
    protected function EventAjaxReactivation()
    {
        ...
        if (getRequestStr('email') and $oUser = $this->User_GetUserByEmail(getRequestStr('email'))) {
            if ($oUser->getActivate()) {
    ...
    protected function EventLogin()
    {
        ...
        $this->Viewer_Assign('sRedirectUrl', getRequestStr('return-path'));
        ...
        if (func_check($this->GetParam(0), 'md5')) {
            ...
            if ($oReminder = $this->User_GetReminderByCode($this->GetParam(0))) {
                if (!$oReminder->getIsUsed() and strtotime($oReminder->getDateExpire()) > time() and $oUser = $oReminder->getUser()) {
                    $this->Viewer_Assign('code', $oReminder->getCode());
                    if (isPost()) {
                        $this->Viewer_SetResponseAjax('json');
                        $password = getRequestStr('password');
                        $passwordConfirm = getRequestStr('password_confirm');
                        ...
                        if ($password == $passwordConfirm) {
                            ...
                            if ($oUser->Update()) {
                                ...
                                $oReminder->Update();
                                $this->User_SendNotifyReminderPassword($oUser, $password);
                                $this->User_Authorization($oUser);
                                $this->Viewer_AssignAjax('sUrlRedirect', '/');
                            }
                            return;
                        } else {
                            $this->Message_AddErrorSingle('Пароли не совпадают');
                            return;
                        }
                    } else {
                        $this->SetTemplateAction('reset_password');
                        return;
                    }
//
                }
    ...
    protected function EventAjaxRegister()
    {
         ...
         */
        $oUser->setEmail(getRequestStr('email'));
