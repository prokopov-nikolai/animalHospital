...
class MapperORM extends Mapper
{
    ...
    public function GetAggregateFunctionByFilter($sAggregateFunction, $sField, $aFilter, $sEntityFull)
    {
        ...
        list($sOrder, $sLimit, $sGroup, $sSelectm, $sHaving) = $this->BuildFilterMore($aFilter, $oEntitySample);
        ...
        $sql = "SELECT {$sAggregateFunction}({$sField}) as value FROM " . $sTableName . " t {$sJoinTables} WHERE 1=1 {$sFilterFields} {$sGroup} {$sHaving}";
    ...
    public function GetItemsByFilter($aFilter, $sEntityFull)
    {
        ...
        list($aFilterFields, $sFilterFields, $sJoinTables) = $this->BuildFilter($aFilter, $oEntitySample);
        list($sOrder, $sLimit, $sGroup, $sSelect, $sHaving) = $this->BuildFilterMore($aFilter, $oEntitySample);

        if (!$sSelect) {
            $sSelect = 't.*';
        }

        $sql = "SELECT {$sSelect} FROM " . $sTableName . " t {$sJoinTables} WHERE 1=1 {$sFilterFields} {$sGroup} {$sHaving} {$sOrder} {$sLimit} ";
        $aQueryParams = array_merge(array($sql), array_values($aFilterFields));
        $aItems = array();
    ...
    public function BuildFilterMore($aFilter, $oEntitySample)
    {
        ...
        $sHaving = '';
        if (isset($aFilter['#having']) && $aFilter['#having']) {
            $sHaving = "HAVING {$aFilter['#having']}";
        }
        ...
        return array($sOrder, $sLimit, $sGroup, $sSelect, $sHaving);
    }
