...
//        $sFieldsJoinReturn = $this->GetFieldsRelationTarget();
        $sql = "SELECT * FROM " . Config::Get('db.table.media') ." t
                WHERE
                	t.target_id = ?d
                	AND
                	t.target_type = ?
                	{ AND t.user_id = ?d }
                ORDER BY
                    t.sort ASC, t.id desc
				limit 0,500";

        $aResult = array();
        if ($aRows = $this->oDb->select($sql, $iTargetId, $sTargetType, $iUserId ? $iUserId : DBSIMPLE_SKIP)) {
     ...
     * @return array
     */
    public function GetMediaByTargetAndFilePath($sTargetType, $sFilePath, $iUserId = null)
    {
        $sql = "SELECT
                    m.*
                FROM " . Config::Get('db.table.media') . " as m
                WHERE
                    m.user_id = ?d
                        AND
                	m.file_path = ?
        ...
        $aResult = array();
        if ($aRows = $this->oDb->select($sql, $iUserId, $sFilePath, $sTargetType)) {
            $aResult = $this->PrepareResultTarget($aRows);
        }
        return $aResult;
    }
...
//        }
        $aResult = [];
        foreach ($aRows as $aRow) {
            $oEntity = Engine::GetEntity('ModuleMedia_EntityMedia', $aRow);
            $oEntity->_SetIsNew(false);
            $aResult[] = $oEntity;
        }
        return $aResult;
