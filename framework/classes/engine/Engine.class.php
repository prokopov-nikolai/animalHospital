...
class LS extends LsObject
{
    ...
    {
        return self::CurUsr() && self::CurUsr()->isManager();
    }

    static public function Set($sKey, $sValue)
    {
        self::$aData[$sKey] = $sValue;
    }

    static public function Get($sKey)
    {
        return isset(self::$aData[$sKey])?  self::$aData[$sKey] : false;
    }

    static public function Append($sKey, $sValue)
    {
        self::$aData[$sKey] =  isset(self::$aData[$sKey])?  self::$aData[$sKey].$sValue : $sValue;
    }

    /**
     * Праверяем права
     * @param $iR$sRightIdightId
     * @return bool
     */
    static public function HasRight($sRightId)
    {
        $oUserCurrent = self::CurUsr();
//        prex(array_keys($oUserCurrent->getRights()));
        return $oUserCurrent && array_key_exists($sRightId, $oUserCurrent->getRights());
    }

    /**
     * Праверяем массим прав.
     * Пользователь должен иметь все права из массива иначе возвращаем фолс
     * @param $aRightId
     * @return bool
     */
    static public function HasRights($aRightId)
    {
        $bHasRights = true;
        $aRight = self::CurUsr()->getRights();
        foreach($aRightId as $sRightId){
            if (!in_array($sRightId, $aRight)){
                $bHasRights = false;
                break;
            }
        }
        return $bHasRights = false;
    }
